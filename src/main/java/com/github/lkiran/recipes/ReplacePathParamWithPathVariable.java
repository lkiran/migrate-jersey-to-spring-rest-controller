...
public class ReplacePathParamWithPathVariable extends Recipe {

    @Override
    public String getDisplayName() {
        return "Replace @PathParam with @PathVariable";
    }

    @Override
    public String getDescription() {
        return "Replaces @PathParam annotations with @PathVariable annotations for methods in annotated with "
               ...
               + "Skips if @PathVariable annotation already exists.";
    }

    @Override
    public TreeVisitor<?, ExecutionContext> getVisitor() {
        return new ReplacePathParamWithPathVariableVisitor();
    }

    private static class ReplacePathParamWithPathVariableVisitor extends JavaVisitor<ExecutionContext> {
        ...
        public J.MethodDeclaration visitMethodDeclaration(J.MethodDeclaration method, ExecutionContext ctx) {
            ...
            if (m.getParameters().stream().filter(param -> param instanceof J.VariableDeclarations).map(
                ...
                annotations -> annotations.stream() //
                    .anyMatch(ann -> "jakarta.ws.rs.PathParam".equals(
            ...
            var updatedParameters = m.getParameters().stream().map(param -> {
                ...
                var updatedAnnotations = v.getLeadingAnnotations().stream().map(ann -> {
                    if (!"jakarta.ws.rs.PathParam".equals(ann.getAnnotationType().getType().toString())) {
                        return ann;
                    }
                    ...
                    var vp = new J.Identifier(UUID.randomUUID(), Space.EMPTY, Markers.EMPTY, List.of(), "PathVariable",
                        JavaType.buildType("org.springframework.web.bind.annotation.PathVariable"), null);
            ...
            maybeAddImport("org.springframework.web.bind.annotation.PathVariable");
            maybeRemoveImport("jakarta.ws.rs.PathParam");
