...
public class ReplaceGetAndPath extends Recipe {

    @Override
    public String getDisplayName() {
        return "Replace @GET and @Path with @GetMapping";
    }

    @Override
    public String getDescription() {
        return "Replace @GET and @Path(\"xxx\") with @GetMapping(\"xxx\") where \"xxx\" can be any value.";
    }

    @Override
    public TreeVisitor<?, ExecutionContext> getVisitor() {
        return new ReplaceGetAndPathVisitor();
    }

    private static class ReplaceGetAndPathVisitor extends JavaVisitor<ExecutionContext> {
        ...
        public J.MethodDeclaration visitMethodDeclaration(J.MethodDeclaration method, ExecutionContext ctx) {
            ...
            var oldMethodAnnotation = m.getLeadingAnnotations().stream().filter(
                a -> a.getAnnotationType().getType().toString().equals("jakarta.ws.rs.GET")).findFirst();
        ...
        private J.@NotNull MethodDeclaration methodWithoutPath(J.Annotation oldMethodAnnotation,
            ...
            var getMappingAnnotation = oldMethodAnnotation.withAnnotationType(
                new J.Identifier(UUID.randomUUID(), Space.EMPTY, Markers.EMPTY, List.of(), "GetMapping",
                    JavaType.buildType("org.springframework.web.bind.annotation.GetMapping"), null));
