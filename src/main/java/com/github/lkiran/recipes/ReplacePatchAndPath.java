...
public class ReplacePatchAndPath extends Recipe {

    @Override
    public String getDisplayName() {
        return "Replace @PATCH and @Path with @PatchMapping";
    }

    @Override
    public String getDescription() {
        return "Replace @PATCH and @Path(\"xxx\") with @PatchMapping(\"xxx\") where \"xxx\" can be any value.";
    }

    @Override
    public TreeVisitor<?, ExecutionContext> getVisitor() {
        return new ReplacePatchAndPathVisitor();
    }

    private static class ReplacePatchAndPathVisitor extends JavaVisitor<ExecutionContext> {
        ...
        public J.MethodDeclaration visitMethodDeclaration(J.MethodDeclaration method, ExecutionContext ctx) {
            ...
            var oldMethodAnnotation = m.getLeadingAnnotations().stream().filter(
                a -> a.getAnnotationType().getType().toString().equals("jakarta.ws.rs.PATCH")).findFirst();
        ...
        private J.@NotNull MethodDeclaration methodWithoutPath(J.Annotation oldMethodAnnotation,
            ...
            var getMappingAnnotation = oldMethodAnnotation.withAnnotationType(
                new J.Identifier(UUID.randomUUID(), Space.EMPTY, Markers.EMPTY, List.of(), "PatchMapping",
                    JavaType.buildType("org.springframework.web.bind.annotation.PatchMapping"), null));
