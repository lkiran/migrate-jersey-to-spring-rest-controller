...
public class ReplaceDeleteAndPath extends Recipe {

    @Override
    public String getDisplayName() {
        return "Replace @DELETE and @Path with @DeleteMapping";
    }

    @Override
    public String getDescription() {
        return "Replace @DELETE and @Path(\"xxx\") with @DeleteMapping(\"xxx\") while preserving the attributes.";
    }

    @Override
    public TreeVisitor<?, ExecutionContext> getVisitor() {
        return new ReplaceDeleteAndPathVisitor();
    }

    private static class ReplaceDeleteAndPathVisitor extends JavaVisitor<ExecutionContext> {
        ...
        public J.MethodDeclaration visitMethodDeclaration(J.MethodDeclaration method, ExecutionContext ctx) {
            ...
            var oldMethodAnnotation = m.getLeadingAnnotations().stream().filter(
                a -> a.getAnnotationType().getType().toString().equals("jakarta.ws.rs.DELETE")).findFirst();
        ...
        private J.@NotNull MethodDeclaration methodWithoutPath(J.Annotation oldMethodAnnotation,
            ...
            var getMappingAnnotation = oldMethodAnnotation.withAnnotationType(
                new J.Identifier(UUID.randomUUID(), Space.EMPTY, Markers.EMPTY, List.of(), "DeleteMapping",
                    JavaType.buildType("org.springframework.web.bind.annotation.DeleteMapping"), null));
