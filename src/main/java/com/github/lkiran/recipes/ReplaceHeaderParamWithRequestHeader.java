...
public class ReplaceHeaderParamWithRequestHeader extends Recipe {

    @Override
    public String getDisplayName() {
        return "Replace @HeaderParam with @RequestHeader";
    }

    @Override
    public String getDescription() {
        return "Replaces @HeaderParam annotations with @RequestHeader annotations for methods in annotated with "
               ...
               + "Skips if @RequestHeader annotation already exists.";
    }

    @Override
    public TreeVisitor<?, ExecutionContext> getVisitor() {
        return new ReplaceHeaderParamWithRequestHeaderVisitor();
    }

    private static class ReplaceHeaderParamWithRequestHeaderVisitor extends JavaVisitor<ExecutionContext> {
        ...
        public J.MethodDeclaration visitMethodDeclaration(J.MethodDeclaration method, ExecutionContext ctx) {
            ...
            if (m.getParameters().stream().filter(param -> param instanceof J.VariableDeclarations).map(
                ...
                annotations -> annotations.stream() //
                    .anyMatch(ann -> "jakarta.ws.rs.HeaderParam".equals(
            ...
            var updatedParameters = m.getParameters().stream().map(param -> {
                ...
                var updatedAnnotations = v.getLeadingAnnotations().stream().map(ann -> {
                    if (!"jakarta.ws.rs.HeaderParam".equals(ann.getAnnotationType().getType().toString())) {
                        return ann;
                    }
                    ...
                    var vp = new J.Identifier(UUID.randomUUID(), Space.EMPTY, Markers.EMPTY, List.of(), "RequestHeader",
                        JavaType.buildType("org.springframework.web.bind.annotation.RequestHeader"), null);
            ...
            maybeAddImport("org.springframework.web.bind.annotation.RequestHeader");
            maybeRemoveImport("jakarta.ws.rs.HeaderParam");
