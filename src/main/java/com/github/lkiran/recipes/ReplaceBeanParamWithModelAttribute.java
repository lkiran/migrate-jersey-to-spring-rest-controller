...
public class ReplaceBeanParamWithModelAttribute extends Recipe {

    @Override
    public String getDisplayName() {
        return "Replace @BeanParam with @ModelAttribute";
    }

    @Override
    public String getDescription() {
        return "Replaces @BeanParam with @ModelAttribute  annotation from method parameters for methods in annotated with "
               + "@GetMapping, @PostMapping, @PutMapping, @PatchMapping, and @DeleteMapping.";
    }

    @Override
    public TreeVisitor<?, ExecutionContext> getVisitor() {
        return new ReplaceBeanParamWithModelAttributeVisitor();
    }

    private static class ReplaceBeanParamWithModelAttributeVisitor extends JavaVisitor<ExecutionContext> {
        ...
        public J.MethodDeclaration visitMethodDeclaration(J.MethodDeclaration method, ExecutionContext ctx) {
            ...
            if (m.getParameters().stream().filter(param -> param instanceof J.VariableDeclarations).map(
                ...
                annotations -> annotations.stream() //
                    .anyMatch(ann -> "jakarta.ws.rs.BeanParam".equals(
            ...
            var updatedParameters = m.getParameters().stream().map(param -> {
                ...
                var updatedAnnotations = v.getLeadingAnnotations().stream().map(ann -> {
                    if (!"jakarta.ws.rs.BeanParam".equals(ann.getAnnotationType().getType().toString())) {
                        return ann;
                    }
                    ...
                    var vp = new J.Identifier(UUID.randomUUID(), Space.EMPTY, Markers.EMPTY, List.of(), "ModelAttribute",
                        JavaType.buildType("org.springframework.web.bind.annotation.ModelAttribute"), null);

                    ann = ann.withAnnotationType(vp);
                    return ann;
                }).toList();
            ...
            maybeRemoveImport("jakarta.ws.rs.BeanParam");
            maybeAddImport("org.springframework.web.bind.annotation.ModelAttribute");
